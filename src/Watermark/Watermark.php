...
class Watermark implements PluginInterface, EventSubscriberInterface
{
    /**
     * @var CKFinder
     */
    ...
    {
        return array(
            'watermarkImagePath' => __DIR__ . '/stamp.png',
            'position' => array(
                'right' => 0,
                'bottom' => 0
            )
    ...
    public function calculatePosition(Image $uploadedImage, Image $watermarkImage)
    {
        ...
        $position = Config::get('Watermark.position');
    ...
    public function addWatermark(FileUploadEvent $event)
    {
        ...
        if (Image::isSupportedExtension($uploadedFile->getExtension()) && $uploadedFile->isValidImage()) {
            $uploadedImage = Image::create($uploadedFile->getData());
            $uploadedImageGD = $uploadedImage->getGDImage();

            $watermarkImagePath = Config::get('Watermark.watermarkImagePath');
